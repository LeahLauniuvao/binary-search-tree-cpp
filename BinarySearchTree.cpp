...
void BinarySearchTree::Insert(Bid bid) {
	...
	if (root == nullptr) {
		...
		this->addNode(root, bid);  // if bid is not null
	}
}
...
Bid BinarySearchTree::Search(string bidId) {
	...
	// Make root node current node pointer
	// Start searching from the root
	Node* current = root; // Assign root to current node pointer
...
void BinarySearchTree::addNode(Node* node, Bid bid) {
	...
	if (node->bid.bidId.compare(bid.bidId) > 0) {
		if (node->left == nullptr) {  // if no left pointer
			node->left = new Node(bid);  // create new left node
		} else {
			this->addNode(node->left, bid);
		}
	}
	// Add to right subtree if left node is not null: traverse down
	else{
		if (node->right == nullptr) {  // if no right pointer
			node->right = new Node(bid);  // create new right node
		} else {
